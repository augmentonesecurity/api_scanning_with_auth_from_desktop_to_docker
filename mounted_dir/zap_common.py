...
@hook(wrap=True)
def zap_active_scan(zap, target, policy, contextId):
    logging.debug('Active Scan ' + target + ' with policy ' + policy)
    ascan_scan_id = zap.ascan.scan_as_user(target, recurse=True, scanpolicyname=policy, contextid=contextId, userid=68)
    time.sleep(5)
